...
<div class="breadcrumb-header justify-content-between">
    ...
    @can('product-create')
        <div class="d-flex my-xl-auto right-content">
            <a class="btn btn-primary btn-block" href="{{ route('settings.create') }}">اضافه الاعدادات</a>
        </div>
    @endcan
</div>
<!-- breadcrumb -->
...
@if (session()->has('edit'))
    <script>
        window.onload = function() {
            notif({
                msg: " تم تحديث بيانات الاعدادات بنجاح",
                type: "success"
            });
        }
    </script>
                    ...
                    <table class="table table-hover  text-md-nowrap" id="example1" data-page-length='50'
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="wd-15p border-bottom-0"> فاكس</th>
                                <th class="wd-15p border-bottom-0">  لينك الخريطة  </th>
                                <th class="wd-15p border-bottom-0">  لينك  الفيس بوك</th>
                                <th class="wd-15p border-bottom-0"> لينك اليوتيوب يوب   </th>
                        ...
                        <tbody>
                            @foreach ($settings as $setting)
                                <tr>
                                    ...
                                    <td>{{ $setting->site_fax }}</td>
                                    <td>{{ $setting->site_map }}</td>
                                    <td>{{ $setting->site_facebook }}</td>
                                    <td>{{ $setting->site_youtube }}</td>
...
data-user_id="{{ $setting->id }}"
data-toggle="modal" href="#modaldemo8" title="حذف">
<i class="las la-trash"></i>
</a>

<!-- Modal Structure -->
<div class="modal fade" id="modaldemo8" tabindex="-1" role="dialog" aria-labelledby="modaldemo8Title" aria-hidden="true">
 <div class="modal-dialog modal-dialog-centered" role="document">
     <div class="modal-content">
         <div class="modal-header">
             <h6 class="modal-title" id="modaldemo8Title">حذف الاعدادات</h6>
             <button aria-label="Close" class="close" data-dismiss="modal" type="button">
                 <span aria-hidden="true">&times;</span>
             </button>
         </div>
         <form id="deleteForm" action="{{route('settings.destroy',0)}}" method="POST">
             @csrf
             @method('DELETE')
    ...
    <div class="modal" id="modaldemo8">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content modal-content-demo">
                ...
                <form action="{{ route('settings.destroy', 0) }}" method="post">
                    {{ method_field('delete') }}
                    {{ csrf_field() }}
                    <div class="modal-body">
                        <p>هل انت متاكد من عملية الحذف ؟</p><br>
                        <input type="hidden" name="setting_id" id="user_id" value="">
                        <input class="form-control" name="username" id="username" type="text" readonly>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">الغاء</button>
...
$('#modaldemo8').on('show.bs.modal', function (event) {
        ...
        var userId = button.data('user_id'); // Extract info from data-* attributes
        ...
        // Update the form action to include the correct user ID
        var action = '{{ route("settings.destroy", ":id") }}';
