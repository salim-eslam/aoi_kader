...
<div class="breadcrumb-header justify-content-between">
    <div class="my-auto">
        <div class="d-flex">
            <h4 class="content-title mb-0 my-auto">التخفيضات</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ قائمة
                التخفيضات</span>
        </div>
    </div>
    <div class="d-flex my-xl-auto right-content">
        <a class="btn btn-primary btn-block" href="{{ route('offers.create') }}">اضافه تخفيض</a>
    </div>
</div>
<!-- breadcrumb -->
@endsection
                    ...
                    <table class="table table-hover  text-md-nowrap" id="example1" data-page-length='50' style=" text-align: center;">
                        <thead>
                            <tr>
                                ...
                                <th class="wd-15p border-bottom-0">عنوان التخفيض</th>
                                <th class="wd-15p border-bottom-0">قيمه التخفيض</th>
                                <th class="wd-15p border-bottom-0">حالة التخفيض</th>
                            </tr>
                        </thead>
                        <tbody>
                            @if ($offers->count()>0)
                             @foreach ($offers as $offer)
                                <tr>
                                    <td>{{ $offer->id }}</td>
                                    <td>{{ $offer->getTranslation('title', 'ar')  }}</td>
                                    <td>{{ $offer->value }}</td>
                                    <td>
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ route('offers.edit', $offer->id) }}" class="btn btn-sm btn-info"
                                        ...
                                        data-user_id="{{ $offer->id }}" data-username="{{ $offer->title }}"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">حذف التخفيض</h6><button aria-label="Close" class="close"
                        data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                </div>
                <form action="{{ route('offers.destroy',0) }}" method="post">
                    {{ method_field('delete') }}
                    {{ csrf_field() }}
                    <div class="modal-body">
                        <p>هل انت متاكد من عملية الحذف ؟</p><br>
                        <input type="hidden" name="offer_id" id="user_id" value="">
