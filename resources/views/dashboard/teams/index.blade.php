...
<div class="breadcrumb-header justify-content-between">
    <div class="my-auto">
        <div class="d-flex">
            <h4 class="content-title mb-0 my-auto">الاعضاء</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ قائمة
                الاعضاء</span>
        </div>
    </div>
    @can('team-create')
        <div class="d-flex my-xl-auto right-content">
            <a class="btn btn-primary btn-block" href="{{ route('teams.create') }}">اضافه عضو</a>
        </div>
    @endcan
</div>
<!-- breadcrumb -->
                        ...
                        <tbody>
                            @foreach ($teams as $team)
                                <tr>
                                    ...
                                    <td>
                                        @can('team-edit')
                                            <a href="{{ route('teams.edit', $team->id) }}" class="btn btn-sm btn-info"
                                                title="تعديل"><i class="las la-pen"></i></a>
                                        @endcan
                                        @can('team-delete')
                                            <a class="modal-effect btn btn-sm btn-danger" data-effect="effect-scale"
                                                data-user_id="{{ $team->id }}" data-username="{{ $team->name }}"
    ...
    <div class="modal" id="modaldemo8">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content modal-content-demo">
                ...
                <form action="{{ route('teams.destroy', 0) }}" method="post">
                    {{ method_field('delete') }}
                    {{ csrf_field() }}
                    <div class="modal-body">
                        <p>هل انت متاكد من عملية الحذف ؟</p><br>
                        <input type="hidden" name="team_id" id="user_id" value="">
